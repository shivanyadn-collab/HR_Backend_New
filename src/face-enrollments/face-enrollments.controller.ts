...
export class FaceEnrollmentsController {
  ...
  async uploadImage(
    ...
    try {
      // Store in S3 bucket same way as employee-documents (folder: face-enrollments; key generated by bucket service)
      const uploadResult = await this.bucketService.uploadFile(file, 'face-enrollments')

      const uploadDto: UploadFaceImageDto = {
        faceEnrollmentId: id,
        imageUrl: uploadResult.url,
        imageKey: uploadResult.key,
        imageName: file.originalname || uploadResult.key.split('/').pop() || 'face.jpg',
        imageSize: file.size,
      }
      return this.service.uploadFaceImage(uploadDto)
    } catch (error) {
      throw new BadRequestException(`Failed to upload face image to S3: ${error.message}`)
    }
  }
